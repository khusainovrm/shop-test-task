<template>
  <div class="product-container gridOnSmall gridOn1150 gridOn1083">
    <div v-for="product in products" :key="product.id" class="product">
      <div class="inner-container">
        ...
        <div v-if="product.rating" class="raiting">
          {{ product.rating }}
        </div>

        <div class="icon" @click="addToCart(product)">
...
export default {
  async fetch () {
    const { data } = await this.$axios.get('http://front-test.idalite.com/api/product')
  ...
  computed: {
    products () {
      const category = this.$store.getters['selectedCategory/selectedCategory']
      const sort = this.$store.getters['sort/sortedBy']
      const selectedByCategory = this.$store.getters['products/products'].filter(product => product.category === category)
      const selectedBySort = selectedByCategory.sort((a, b) => {
        if (sort === 'price') {
